...
import { generateDomainFromName } from '@/utils'
...
function AddProjectModalHelper({
  ...
  useEffect(() => {
    if (debouncedDomain.length > 0) {
      fetch(`/api/projects/douni.one/domains/${debouncedDomain}/exists`).then(
        async (res) => {
          if (res.status === 200) {
            const exists = await res.json()
        ...
        >
          ...
          <div>
            ...
            <div className="relative mt-1 flex rounded-md shadow-sm">
              <span className="h-[38px] inline-flex items-center rounded-l-md border border-r-0 border-gray-300 bg-gray-50 px-5 text-gray-500 sm:text-sm">
                app.douni.one
              </span>
              <input
                name="slug"
                ...
                } inline-flex items-center block w-full appearance-none rounded-r-md border border-gray-300 px-3 h-[38px] placeholder-gray-400 shadow-sm focus:border-black focus:outline-none focus:ring-black sm:text-sm`}
                placeholder="dounione"
                value={slug}
                onChange={(e) => {
                  setSlugError(null);
            ...
            <div className="relative mt-1 flex rounded-md shadow-sm">
              <input
                ...
                className={`${
                  domainError
                    ? 'border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500'
                    : 'border-black text-gray-900 placeholder-gray-300 focus:border-gray-500 focus:ring-gray-500'
